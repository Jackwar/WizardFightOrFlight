...
class FireBall {
    ...
    static func createFireBallGameScene(scene: SKScene, sorlos: SKSpriteNode, parentNode: SKNode) {
        
        setupFireBall(sorlos: sorlos)
        
        fireBall.physicsBody?.collisionBitMask = 1
        
        fireBall.physicsBody?.categoryBitMask = projectileGroup
        
        fireBall.physicsBody?.contactTestBitMask = UInt32(8)
        
        parentNode.addChild(fireBall)
        
        fireBall.physicsBody?.applyImpulse(CGVector(dx: 10, dy: 0))
        
    }
    
    static func createFireBallBossScene(scene: SKScene, sorlos: SKSpriteNode, parentNode: SKNode) {
        ...
        fireBall.physicsBody?.contactTestBitMask = UInt32(14)
    ...
    static func setupFireBall(sorlos: SKSpriteNode) {
        
        fireBallCreated = SKTexture(imageNamed: "img/fireBallCreated.png")
        fireBall1 = SKTexture(imageNamed: "img/fireBall1.png")
        fireBall2 = SKTexture(imageNamed: "img/fireBall2.png")
        fireBall3 = SKTexture(imageNamed: "img/fireBall3.png")
